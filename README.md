...
> 当前版本：靓号模式逐项填写最低长度，填 10 表示 10～34 位、留空关闭；每项显示示例及参考耗时。新增数字循环顺子和连续纯数字，周期/回文拆分纯数字与含字母规则。前后缀模式保留尾部快速路径。
> 正常启动默认 M=24，取消 M 比较测速和批次预热；保留正确性自检，编译失败才回退 M=16/8。
> CPU 分类已优化为结构扫描，并通过后台任务和有界缓冲与 GPU 调度并行；按可用 CPU 自动设置分类进程数。
